...
import { Bell, ChevronsLeft, Command, Search } from "lucide-react";
...
import { MOBILE_NAVIGATION, NAVIGATION_GROUPS } from "@/config/navigation";
    ...
    <TooltipProvider delayDuration={200}>
      <div className="min-h-screen flex w-full bg-background">
        ...
        >
          ...
          <nav className="flex-1 px-2.5 py-4 overflow-y-auto" aria-label="Navegação principal">
            {NAVIGATION_GROUPS.map((group, groupIndex) => (
              <div key={group.id} className={cn(groupIndex > 0 && "mt-5")}>
                {!collapsed && <div className="px-2.5 mb-1.5 orbe-eyebrow text-[10px]">{group.label}</div>}
                {collapsed && groupIndex > 0 && <div className="orbe-hairline mx-2 mb-3" />}
                <div className="space-y-0.5">
                  {group.items.map((item) => {
                    ...
                    const link = (
                      ...
                      >
                        {active && <span className="absolute left-0 top-1/2 -translate-y-1/2 h-5 w-[3px] rounded-full bg-[var(--orbe-blue)]" />}
        ...
        <div className="flex-1 flex flex-col min-w-0">
          <header className="h-16 border-b bg-background/70 backdrop-blur-xl sticky top-0 z-30 flex items-center gap-3 px-4 md:px-6">
            ...
            <div className="relative ml-auto w-full max-w-sm hidden sm:block">
              ...
              <Input placeholder="Buscar projetos, chats e conhecimento…" className="pl-9 pr-12 h-9 bg-card/60 border-border/70 focus-visible:bg-card" />
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-56">
                ...
                <DropdownMenuSeparator />
                <DropdownMenuItem asChild><Link to="/app/settings">Configurações</Link></DropdownMenuItem>
                <DropdownMenuItem asChild><Link to="/app/memory">Memória</Link></DropdownMenuItem>
                <DropdownMenuItem asChild><Link to="/app/admin">Administração</Link></DropdownMenuItem>
                <DropdownMenuSeparator />
                <DropdownMenuItem onSelect={(event) => { event.preventDefault(); void handleLogout(); }}>Sair da orbeAI</DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
          </header>
        ...
        <nav className="md:hidden fixed bottom-0 inset-x-0 z-40 border-t bg-background/90 backdrop-blur-xl flex items-center justify-around px-2 py-2" aria-label="Navegação móvel">
          {MOBILE_NAVIGATION.map((item) => {
            const Icon = item.icon;
            const active = isActive(item.to, item.exact);

            return (
              <Link key={item.id} to={item.to as any} className={cn("flex flex-col items-center gap-0.5 text-[10px] font-medium px-2 py-1 rounded-lg transition-colors", active ? "text-[var(--orbe-blue)]" : "text-muted-foreground")}>
